...
// ✅ Chart States
const isChartLoading = computed(() => isLoading.value);

const isChartEmpty = computed(() => {
  if (!chartData.value) return true;
  const ds = chartData.value.datasets?.[0]?.data || [];
  return ds.length === 0 || ds.every((v) => Number(v) === 0);
});
  ...
  <div class="p-6 space-y-6" v-if="!isLoading">
    ...
    <div class="bg-white p-4 rounded shadow border-l-4 border-primary">
      <h2 class="text-lg font-semibold">
        {{ user.role === "student" ? "Admission Info" : "Lecturer Info" }}
      </h2>
      ...
      <div class="flex flex-wrap gap-4 md:gap-6 mt-2 text-gray-700">
        ...
        <div v-if="user.role === 'student' && studentYear">
          <span class="font-bold">Current Year:</span> {{ studentYear }}
        </div>
    ...
    <div class="p-4 bg-white rounded-lg shadow-md w-full flex items-center justify-center">
      <!-- 1) Loading -->
      <div v-if="isChartLoading" class="text-center text-gray-500 w-full">
      ...
      <!-- 2) Empty -->
      <div v-else-if="isChartEmpty" class="text-center text-gray-500 w-full py-10">
        <AlertCircle class="h-10 w-10 text-gray-400 mx-auto mb-3" />
        <div class="text-sm font-semibold text-gray-700">No data to display</div>
        <p class="mt-1 text-xs">
          {{
            user.role === "student"
              ? "No subjects were found for this student."
              : "No assigned subjects/sections were found for this lecturer."
          }}
        </p>
      </div>
      ...
      <template v-else>
        <div class="bg-white border rounded-lg shadow-sm p-4 col-span-full">
          <div class="flex justify-between border-b pb-2 mb-4">
            ...
            <span class="text-xs font-semibold bg-gray-200 px-2 py-1 rounded">
              Total Sections: {{ lecturerSubjects.length }}
            </span>
          </div>

          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div
              v-for="sub in lecturerSubjects"
              :key="sub.kod_subjek + sub.seksyen"
              class="p-4 border rounded-md bg-gray-50 hover:border-primary transition-colors"
            >
              <div class="flex justify-between items-start mb-2">
                <span class="text-xs font-bold text-primary px-2 py-0.5 bg-red-50 rounded">
                  Section {{ sub.seksyen }}
                </span>
                <span class="text-[10px] text-gray-400 font-mono">{{ sub.kod_subjek }}</span>
              </div>

              <p class="text-sm font-bold text-gray-800 line-clamp-2 min-h-[40px]">
                {{ sub.nama_subjek }}
              </p>
